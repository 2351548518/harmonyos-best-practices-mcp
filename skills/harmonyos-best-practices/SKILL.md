...
- **中文功能词 + 场景词**:最佳实践按场景组织,用功能描述命中。例:`长列表 丢帧`、`冷启动 优化`、`内存泄漏 检测`、`组件复用`、`音频焦点 并发`。
- **API 名 + 场景**:带 API 名更准。例:`AVPlayer 短视频`、`AVRecorder 录制 音频`、`TaskPool 多线程`。
- **同义词已内置扩展**:列表↔list、弹窗↔dialog、滑动↔滚动、启动↔拉起、卡顿↔掉帧↔丢帧 等自动 OR 扩展,用任一写法都能命中。
- **优先找带 ✅有代码 的**:返回标注 `✅有代码` 的文档有配套示例可参照,无代码的多为理论/检测指导。
- **查不到时**:先 `list_by_topic` 看大类找方向,再下钻(如 `性能 / 性能场景优化案例`)。

## 大类清单(先定位,再下钻)

452 篇文档,29 个大类(括号为文档数):

稳定性(76)、性能(64)、媒体(49)、功耗(47)、一次开发多端部署(47)、应用框架(36)、自由流转(19)、布局与弹窗(18)、多端设备体验提升(14)、行业场景解决方案(13)、图形(10)、应用安全(10)、技术创新(7)、主题与样式(5)、网络(4)、组件封装与复用(4)、NDK开发(4)、场景创新(3)、动画与转场(3)、声明式语法(3)、手势与导航(3)、应用架构(3)、编译构建(2)、应用服务(2)、AI使能(2)、测试框架(1)、概览(1)、文档变更说明(1)、应用质量概览(1)。

用 `list_by_topic({topic:"<大类>"})` 下钻,支持多级路径前缀(如 `媒体 / 音频和视频`,括号容错)。
